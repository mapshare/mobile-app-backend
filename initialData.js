...
module.exports = io => {
  ...
  return {
    initialData: async () => {
      const AdminRole = await GroupRole.findOne({
      ...
      for (group of groups) {
        const setupNamespace = await ChatData.setupGroupNamespace(
          group._id,
          io
        );
        const setupGroupFeedNamespace = await GroupFeedData.setupGroupFeedNamespace(
          group._id,
          io
        );
      }
